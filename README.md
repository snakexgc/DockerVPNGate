...
- 内部支持 HTTP 与 SOCKS5 自适应代理协议。
- 代理目标域名优先通过对应 VPN 隧道使用 DoH 解析，国外解析器失败后才尝试国内解析器，全部 DoH 失败后再回退普通 DNS。
- 五个代理端口共用一组可动态修改的认证凭据。
- 节点有效性通过任务队列和默认 8 个工作线程并发检测；更新节点可取消当前队列，并在新旧节点全量测试后提交地区节点池。
- 节点延时在 OpenVPN 隧道建立后通过该隧道请求 Google `generate_204` 实测，不使用入口服务器 Ping 冒充出口延时。
- 持久化节点缓存池按国家/地区独立限额，默认每地区最多 10 个节点(可设置)；更新时先完成新旧节点全量测试，再优先替换超时节点，其次替换实测延迟最高的旧节点。
- API 拉取失败或返回空结果时不会清空本地节点池；测试/更新异常时会恢复更新前快照。
- 节点列表固定从 `https://www.vpngate.net/api/iphone/` 拉取。
...
下载 Compose 配置并进入项目目录：

```bash
git clone https://github.com/snakexgc/DockerVPNGate.git
cd DockerVPNGate
```
...
如需固定到已发布的 tag，在项目目录创建 `.env` 文件：

```dotenv
DOCKERVPNGATE_IMAGE=ghcr.io/snakexgc/dockervpngate:v1.0.0
```

将 `v1.0.0` 替换为需要部署的版本，然后重新执行 `docker compose pull` 和 `docker compose up -d --no-build`。
...
代理目标域名的解析请求会绑定到该代理对应的 `tun` 网卡，不会通过宿主机物理网络泄漏。解析顺序固定为：

1. `https://dns.cloudflare.com/dns-query`（通过 `1.1.1.1` 引导连接）
2. `https://dns.google/dns-query`（通过 `8.8.8.8` 引导连接）
3. `https://dns.alidns.com/dns-query`（通过 `223.5.5.5` 引导连接）
4. 普通 DNS `1.1.1.1`
5. 普通 DNS `8.8.8.8`
6. 普通 DNS `223.5.5.5`

每一级失败或超时后才会尝试下一级，因此国外 DNS 优先、国内 DNS 兜底。DoH 使用固定引导 IP 避免解析 DoH 服务自身时产生循环，同时仍按域名校验 HTTPS 证书。所有隧道 DNS 都失败时，代理连接会直接失败，不会回退容器或宿主机的系统解析器。

Docker Compose 另外为管理程序、VPNGate API 和 OpenVPN 等普通容器流量配置了 `1.1.1.1`、`8.8.8.8`、`223.5.5.5`。Docker 的 `dns` 配置只接受普通 DNS 服务器 IP，不接受 DoH URL；完整的 DoH 优先链由代理程序负责。
...
节点检测会为每个工作线程建立一条临时 OpenVPN 隧道，是维护期间最主要的 CPU 来源。检测结果会批量写回紧凑格式的 `nodes.json`，避免对每个节点反复解析和重写整个节点池。常驻守护线程会复用节点缓存，日志页面从文件尾部读取，Web 心跳成功请求也不会重复写入访问日志。

这三个参数可以直接在 Web 管理页面的“设置 → 节点检测性能”中修改：

- “节点检测并发数”：默认 8，范围 1–8；下一批节点检测生效。
- “API 最大候选数”：默认 300，范围 1–2000；下一次更新节点生效。
- “每节点自动重测间隔”：默认 10 秒，范围 1–3600；下一轮自动调度时生效。

小型服务器可设置为 `2 / 150 / 30`。并发数和候选数越小，CPU 峰值越低，但全量检测更慢、节点覆盖更少；自动重测间隔越大，平均 CPU 消耗越低，但节点状态更新更慢。这三个参数由 WebUI 统一管理，并持久化到 `vpngate_data/ui_auth.json`。
...
```bash
docker pull ghcr.io/snakexgc/dockervpngate:latest
...
docker run -d \
  ...
  ghcr.io/snakexgc/dockervpngate:latest
```
...
直接运行 Docker 时，配置、地区偏好、节点缓存和日志保存在 `dockervpngate-data` 命名卷中，重建容器不会丢失。

## 开发流程

开发或修改源码时，从仓库根目录使用 Compose 的本地构建配置：
...
`--build` 会使用当前目录的 [Dockerfile](Dockerfile) 重新构建镜像。修改代码后再次执行 `docker compose up -d --build` 即可更新开发容器；生产部署应继续使用前面的 `pull` 和 `--no-build` 流程。
