...
- 节点检测使用可配置工作线程和一个任务队列，默认 8 线程，范围 1–8。`node_test_workers`、`max_scan_rows` 和 `node_auto_retest_seconds_per_node` 必须持久化到 WebUI 配置，分别在下一批检测、下一次拉取和下一轮调度时动态读取，环境变量只作为首次创建配置时的默认值。批量检测状态机必须是 `queued`（排队中）→ `testing`（检测中）→ `available/unavailable`（可用/不可用），实时状态由内存覆盖层提供给前端，完成结果分批持久化，不能恢复为每个节点反复重写整个 `nodes.json`。重复批量测试请求应丢弃；更新节点必须取消现有测试队列，暂存本地与拉取节点并全量测试，测试完成后才按地区限额提交；如果维护任务结束后仍有 `queued/testing/not_checked` 遗留节点，后台需要走续测缓存池流程，不能等待下一次 API 拉取周期。
...
- 后台自动维护在排队节点清空后按“当前节点池节点数 × WebUI 每节点自动重测间隔”等待，默认每节点 10 秒；到点后需要拉取合并并全量重测节点池，而不是只补测新增或不可用节点。
...
- 手动单节点检测独立于可配置的批量队列；连续触发时新请求必须取消旧请求，只有最后一次请求允许写回结果。
...
| `vpngate_app/storage.py` | 原子 JSON 读写、紧凑节点存储、按文件版本失效的节点内存缓存、管理配置迁移和代理槽默认值。 |
...
| `vpngate_app/node_testing.py` | 临时测试隧道、Google 204 延时检测、可配置工作队列、批量持久化与取消恢复。 |
| `vpngate_app/traffic.py` | `tun0`–`tun4` 每秒采样、今日/累计流量与原子清零。 |
| `vpngate_app/web_api.py` | 登录会话、静态资源和 REST API 路由。通过入口模块注入运行时服务；成功的高频 Web 心跳不写访问日志。 |
| `vpngate_app/logging_utils.py` | 结构化 JSON 日志、反向分块读取最近日志及过期清理。 |
