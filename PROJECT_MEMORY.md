...
- 节点检测使用 8 个工作线程和一个任务队列。批量检测状态机必须是 `queued`（排队中）→ `testing`（检测中）→ `available/unavailable`（可用/不可用），不要再让前端显示“未检测”。重复批量测试请求应丢弃；更新节点必须取消现有测试队列，完成拉取与合并后再测试；如果维护任务结束后仍有 `queued/testing/not_checked` 遗留节点，后台需要走续测缓存池流程，不能等待下一次 API 拉取周期。
...
4. 新节点与缓存池合并：优先替换不可用节点，再淘汰高延时节点。
5. 启动或手动更新时，缓存池中的全部节点先写为 `queued` 并进入队列；工作线程取到节点后写为 `testing`，完成后写为 `available` 或 `unavailable`。普通周期只把新增、排队或达到复测间隔的节点重新入队。
6. 为五个代理执行 `ensure_proxy_slot()`。
